# INKREALM.CO: ข้อกำหนดระบบและคุณสมบัติหลัก (V5.1) - ฉบับปรับปรุง README (Next.js & Supabase Stack)

เอกสารนี้เป็นข้อกำหนดระบบและคุณสมบัติหลักสำหรับแพลตฟอร์ม INKREALM.CO (V5.1) โดยเน้นฟังก์ชันที่จำเป็นตามการปรับปรุงล่าสุด โดยอ้างอิง Tech Stack ใหม่ (Next.js (React) & Supabase Stack) และออกแบบมาเพื่อความสะดวกในการปรับใช้, ทดสอบ, และย้ายไปยังโดเมนต่างๆ
...
- **ใช้งานง่าย ตอบโจทย์ทุกอุปกรณ์:**
  ...
  - Responsive Design เพื่อประสบการณ์ที่ดีในทุกขนาดหน้าจอ (ใช้ Tailwind CSS utilities)
...
- **การเปลี่ยนธีม:**
  ...
  - ควบคุมผ่านองค์ประกอบ UI ที่ชัดเจน (เช่น ไอคอนพระอาทิตย์/พระจันทร์ จาก Lucide React ที่มีการเปลี่ยนแปลงสถานะชัดเจน) และสถานะธีมต้องถูกบันทึกไว้ในการตั้งค่าของผู้ใช้ (localStorage และ/หรือ `profiles.user_settings` ใน Supabase) อาจใช้ Library เช่น `next-themes` เพื่อความสะดวก
...
- **ระบบจัดการเนื้อหานิยาย (CMS):**
  - การสร้าง/แก้ไข/ลบนิยายและตอน (ดำเนินการผ่าน Modals (Shadcn/ui Dialog) เป็นหลัก เพื่อ UX ที่ต่อเนื่อง)
...
- **ระบบการแจ้งเตือนผู้ใช้ (User Feedback System):** การใช้ Toast notifications (เช่น Shadcn/ui Toaster ที่สร้างบน react-hot-toast หรือ Sonner) สำหรับยืนยันการกระทำ, แจ้งเตือนข้อผิดพลาด, หรือให้ข้อมูลสั้นๆ
...
- **สมาชิก (Member):**
  ...
  - **แถบเครื่องมือสำหรับอ่าน (Reading Toolbar Component):** สารบัญ (เปิด Modal แสดงรายการตอน), ปรับแต่งการแสดงผล (เปิด Popover/Dropdown จาก Shadcn/ui), บุ๊คมาร์คตอนปัจจุบัน (สลับสถานะ), สลับธีมเว็บ (Light/Dark), เปิด/ปิดโหมดอ่านต่อเนื่อง, นำทางตอนก่อนหน้า/ถัดไป
- **นักแปล (Translator):**
  - **ความสามารถหลัก:**
    ...
    - เข้าถึง "โหมดแปล" (เมื่อเปิดใช้งานสำหรับนิยายนั้น): แสดงเลขบรรทัด, checkbox (Shadcn/ui Checkbox), ดับเบิลคลิกที่บรรทัดเนื้อหาเพื่อมาร์คและคัดลอก
    - ใช้งานระบบบรรทัดมาร์ค (Mark Lines) และบันทึกข้อมูลไปยัง Supabase ทันที
    - สร้าง/แก้ไข/ลบนิยายและตอนของตนเอง (นิยายที่ `created_by` ตรงกับ `user_id` ของตน หรือได้รับสิทธิ์จัดการผ่านตาราง permissions แยกต่างหาก ถ้ามี) การดำเนินการผ่าน Modal (Shadcn/ui Dialog)
    - เข้าถึงแดชบอร์ดนักแปล (`/translator/dashboard`) และหน้าจัดการบรรทัดมาร์คสำหรับนิยายของตน
  - **แถบเครื่องมือพิเศษในโหมดแปล (เมื่อเปิด "โหมดนักแปล" สำหรับนิยายนั้น และเข้าหน้าอ่านตอน):** เหมือนของ Member แต่เพิ่ม: ปุ่ม "คัดลอกมาร์คทั้งหมด" (Copy All Marks) สำหรับตอนปัจจุบัน (ใช้ Lucide React icon)
- **ผู้ดูแลระบบ (Admin):**
  - **ความสามารถหลัก:**
    ...
    - จัดการผู้ใช้ทั้งหมด: ดูรายการ, เข้าดู/แก้ไขโปรไฟล์ (ชื่อที่แสดง, รูปโปรไฟล์), เปลี่ยนบทบาท, กำหนดวันหมดอายุสิทธิ์, ระงับบัญชี (ผ่านหน้า User Management `/admin/dashboard/users` และหน้าย่อย `/admin/dashboard/users/[user_id]` หรือ Modal)
...
- **การแสดงผล:**
  - แสดงผลผ่าน **Modal** (Shadcn/ui Dialog) เป็นหลักเมื่อผู้ใช้คลิกปุ่ม "เข้าสู่ระบบ" หรือ "ลงทะเบียน" จากภายใน Navbar หรือส่วนอื่นๆ ของเว็บที่ผู้ใช้ยังไม่ได้ล็อกอิน
...
- **การพัฒนา:**
  ...
  - **Frontend (Next.js):**
    - สร้าง Forms ด้วย React Components และ Shadcn/ui (Input, Button, etc.)
...
- **การแจ้งเตือน (User Feedback):** แสดง Toast notification (เช่น "เข้าสู่ระบบสำเร็จ", "ลงทะเบียนเรียบร้อย กรุณายืนยันอีเมล", "คำขอรีเซ็ตรหัสผ่านถูกส่งไปยังอีเมลของคุณแล้ว", "รหัสผ่านถูกเปลี่ยนสำเร็จ กรุณาเข้าสู่ระบบใหม่") โดยใช้ Shadcn/ui Toaster หรือ react-hot-toast

## 4. โครงสร้างแถบนำทาง (Navbar Structure)

- **ความสามารถ:** แสดงเมนูแบบไดนามิกตามบทบาทของผู้ใช้ (Guest, Member, Translator, Admin), Responsive Design (ปรับการแสดงผลสำหรับ Mobile เช่น ใช้ Shadcn/ui NavigationMenu หรือ Sheet สำหรับเมนูแบบ Hamburger)
- **ตัวอย่างเมนู (สร้างเป็น React Component):**
  - **Guest:**
    - โลโก้ INKREALM.CO (ใช้ `<Link href="/">` จาก `next/link`)
    - (อาจมีลิงก์ "นิยายทั้งหมด" หรือ "หมวดหมู่")
    - ปุ่มสลับธีม (ไอคอน Sun/Moon จาก Lucide React)
    - ปุ่ม "เข้าสู่ระบบ" (Shadcn/ui Button, เปิด Login Modal)
    - ปุ่ม "ลงทะเบียน" (Shadcn/ui Button, เปิด Register Modal)
  - **Member:**
    - โลโก้ INKREALM.CO
    - `<Link href="/">หน้าแรก</Link>`
    - `<Link href="/bookmarks">บุ๊คมาร์ค</Link>`
    - ปุ่มสลับธีม
    - โปรไฟล์ผู้ใช้ (Shadcn/ui DropdownMenu หรือ Avatar + Popover เมื่อคลิกที่ Avatar/ชื่อผู้ใช้):
...
- **การพัฒนา:**
  ...
  - ใช้ Shadcn/ui Components (NavigationMenu, DropdownMenu, Button, Avatar) หรือสร้าง UI เองด้วย Tailwind CSS
...
- **ส่วนแสดงประกาศ (Announcements Display Area - React Component):**
  ...
  - แสดงผลในบริเวณที่เห็นได้ชัดเจน (เช่น Banner ด้านบน) ด้วย Shadcn/ui Alert หรือ Card, มีปุ่ม "X" (Lucide React icon) ให้ผู้ใช้ปิดประกาศ (สถานะการปิดอาจบันทึกใน `localStorage`)
- **ส่วนเครื่องมือหลัก (React Component):**
  - **ช่องค้นหานิยาย:** (Shadcn/ui Input) สำหรับกรอกคำค้น (ชื่อเรื่อง, คำโปรย) การค้นหาอาจทำแบบ real-time (debounced) หรือ on-submit โดยเรียก API Route
  - **ฟิลเตอร์หมวดหมู่ (Category Filter):** (Shadcn/ui Select) แสดงรายการหมวดหมู่ทั้งหมด (ดึงจากตาราง `categories` แบบไดนามิก, มีตัวเลือก "ทุกหมวดหมู่")
  - **ฟิลเตอร์สถานะนิยาย (Novel Status Filter):** (Shadcn/ui Select) เลือกสถานะ ("ต่อเนื่อง", "จบแล้ว", "ยุติการลง", "ทุกสถานะ")
  - **ปุ่มสร้างนิยาย (Create Novel Button):** (สำหรับ Translator/Admin เท่านั้น) (Shadcn/ui Button) ประกอบด้วย [ไอคอน Lucide React เช่น `PlusSquare` หรือ `FilePlus2`] + ข้อความ "สร้างนิยาย". เมื่อคลิก เปิด Modal สร้างนิยาย (ดูข้อ 5.1.1)
- **การแสดงรายการนิยาย (React Component):**
  - รูปแบบ Grid Layout (ใช้ Tailwind CSS grid)
  - **การ์ดนิยาย (Shadcn/ui Card):** แสดง: **รูปปก (อัตราส่วน 3:4, ใช้ `next/image` เพื่อ optimization), ชื่อเรื่อง, จำนวนตอนทั้งหมด**
  - Responsive: PC (เช่น 4-5 คอลัมน์), Tablet (3 คอลัมน์), Mobile (1-2 คอลัมน์)
  - ใช้ `<Link href={`/novels/${novel.id}`}>` ครอบการ์ดเพื่อนำทาง
- **ระบบแบ่งหน้า (Pagination):** (Shadcn/ui Pagination Component) แสดงเมื่อผลลัพธ์การค้นหา/ฟิลเตอร์มีจำนวนมากกว่าที่กำหนดต่อหน้า (เช่น 20 รายการต่อหน้า) การเปลี่ยนหน้าจะอัปเดต Query Parameters และเรียกข้อมูลใหม่
- **การพัฒนา:**
  ...
  - **UI Components:** Shadcn/ui (Input, Select, Button, Card, Pagination), Tailwind CSS
...
- **UI ภายใน Modal (Shadcn/ui Dialog Component):**
  - **หัวเรื่อง Modal:** "สร้างนิยายใหม่"
  - **ฟอร์ม (จัดการด้วย React Hook Form, Validation ด้วย Zod):**
    - **ชื่อเรื่อง (Title):** (Shadcn/ui Input), จำเป็นต้องกรอก (Zod: `required`, `minlength`)
    - **คำโปรย (Synopsis):** **Rich Text Editor** (เช่น TipTap ผูกกับ Controller ของ React Hook Form), (Zod: `optional`, `maxlength`)
    - **หมวดหมู่ (Category):** (Shadcn/ui Select) เลือกจากรายการหมวดหมู่ (ดึงจาก `categories` table), (Zod: `optional` หรือ `required`)
    ...
    - **ภาษาต้นฉบับ (Original Language):** (Shadcn/ui Select) เลือก ("จีน (zh)", "อังกฤษ (en)", "ไทย (th)"), (Zod: `required`)
    - **Slug นิยาย (`novel_slug`):** (Shadcn/ui Input), auto-generate จากชื่อเรื่อง (Client-side logic), ผู้ใช้สามารถแก้ไขได้ (ต้องมีการตรวจสอบความซ้ำซ้อนของ `novel_slug` ผ่าน API call ก่อน Submit)
    - **สถานะนิยาย (Novel Status):** (Shadcn/ui Select หรือ RadioGroup) เลือกจาก ("ต่อเนื่อง", "จบแล้ว", "ยุติการลง"), ค่าเริ่มต้น "ต่อเนื่อง"
    - **สิทธิ์ในการมองเห็นนิยาย (Novel Visibility):** (Shadcn/ui Select (multiple) หรือ Checkbox Group) เลือกได้หลายอัน ("ทุกคน", "เฉพาะสมาชิก", "เฉพาะนักแปล"), ค่าเริ่มต้น "ทุกคน" (Zod: `array`, `min(1)`)
    - **การตั้งค่าเริ่มต้นสำหรับตอนใหม่ของนิยายนี้ (Default Chapter Settings for this Novel):**
      - **สถานะเริ่มต้นของตอน:** (Shadcn/ui RadioGroup) ("เผยแพร่" (ค่าเริ่มต้น), "ฉบับร่าง")
      - **สิทธิ์การเข้าถึงตอนเริ่มต้น:** (Shadcn/ui Select (multiple) หรือ Checkbox Group) ("ทุกคน" (ค่าเริ่มต้น), "เฉพาะสมาชิก", "เฉพาะนักแปล")
  - **ปุ่มดำเนินการ (Shadcn/ui Button):**
    - "เผยแพร่นิยาย": Submit form, เรียก API Route เพื่อสร้างนิยายและตั้งสถานะ (เมื่อสำเร็จ แสดง Toast "สร้างนิยาย '[ชื่อนิยาย]' และเผยแพร่สำเร็จแล้ว")
    - "บันทึกเป็นฉบับร่าง": Submit form, เรียก API Route เพื่อสร้างนิยายและตั้งสถานะเป็น 'draft' (เมื่อสำเร็จ แสดง Toast "สร้างนิยาย '[ชื่อนิยาย]' เป็นฉบับร่างสำเร็จแล้ว")
    - "ยกเลิก" (ปิด Modal)
- **การพัฒนา:**
  - **UI:** Shadcn/ui Dialog, Input, Select, RadioGroup, Checkbox, Button
...
- **ส่วนข้อมูลนิยาย (Novel Information Section - React Component):**
  ...
  - **(สำหรับนักแปล/Admin ที่มีสิทธิ์ในนิยายเรื่องนี้) ปุ่มสลับ "โหมดนักแปล" (Translator Mode Toggle):**
    - (Shadcn/ui Switch) ชัดเจน เช่น "โหมดนักแปล: ปิด/เปิด"
...
- **ปุ่มจัดการนิยาย (สำหรับผู้สร้างนิยาย (`created_by`) / Admin - แสดงตามเงื่อนไข):**
  - ปุ่ม "แก้ไขข้อมูลนิยาย" (Shadcn/ui Button): เปิด Modal แก้ไขข้อมูลนิยาย (ดูข้อ 5.2.1)
  - ปุ่ม "ลบนิยาย" (Shadcn/ui Button, destructive variant): แสดง Modal ยืนยัน (Shadcn/ui AlertDialog) ก่อนเรียก API Route เพื่อลบ (เมื่อลบสำเร็จ แสดง Toast "ลบนิยาย '[ชื่อนิยาย]' เรียบร้อยแล้ว" และ redirect ด้วย `next/router`)
...
- **ส่วนสารบัญ (Table of Contents Section - React Component):**
  - **การแสดงผล:** (Shadcn/ui Accordion) หรือ List แบบ Collapse/Expand เอง, ปรับจำนวนตอนต่อกลุ่มที่แสดงได้ (เช่น Dropdown เลือก 10, 20, 50, 100 ตอน)
  - **รายการตอน (แต่ละตอนใน Accordion Item หรือ List Item):**
    - **(สำหรับทุกคน):**
      - ไอคอนบุ๊คมาร์ค (Lucide React, แสดงสถานะว่าบุ๊คมาร์คตอนนี้ไว้หรือไม่)
      - หมายเลขตอน (`chapter_number`), ชื่อตอน (คลิก `<Link href={`/novels/${novelId}/${chapter.chapter_number}`}>` เพื่อไปหน้าอ่าน)
      - สถานะการอ่าน (ถ้าเคยอ่านแล้ว): แสดงไอคอนรูปตา (Lucide React) หรือ UI อื่นๆ (ข้อมูลจาก `user_chapter_reads`)
    - **(สำหรับผู้สร้างนิยาย / Admin เท่านั้น, เพิ่มเติม):**
      - ไอคอนแสดงสถานะตอน (เช่น "เผยแพร่", "ฉบับร่าง")
      - ปุ่ม "แก้ไขตอน" (ไอคอนดินสอ): เปิด Modal เพิ่ม/แก้ไขตอน (ข้อ 5.2.3) โดยโหลดข้อมูลของตอนนี้มา
      - ปุ่ม "ลบตอน" (ไอคอนถังขยะ): แสดง Modal ยืนยัน, เรียก API Route เพื่อลบตอน, Toast, รีเฟรชสารบัญ (React Query invalidate query)
      - **ปุ่ม "คัดลอกมาร์คทั้งหมด" (Copy All Marks):** (ไอคอน Copy) สำหรับตอนนี้ (หากนักแปลเป็นเจ้าของและมีมาร์คใน `mark_lines` สำหรับตอนนี้)
- **การพัฒนา:** Next.js (Dynamic Routes), Shadcn/ui (Switch, Button, Accordion, Dialog, AlertDialog), Lucide React, Supabase, React Query (สำหรับ Client-side updates และ refetching), DOMPurify (ถ้า render HTML)

### 5.2.1 Modal แก้ไขข้อมูลนิยาย (Edit Novel Modal)

- **การเรียกใช้งาน:** คลิกปุ่ม "แก้ไขข้อมูลนิยาย" ในหน้าข้อมูลนิยาย
- **UI ภายใน Modal (Shadcn/ui Dialog):** ฟอร์ม (React Hook Form + Zod) จะถูกเติมด้วยข้อมูลปัจจุบันของนิยาย (ดึงข้อมูลนิยายเมื่อ Modal เปิด)
  ...
  - **ฟิลด์ที่สามารถแก้ไขได้ (เหมือน 5.1.1 แต่สำหรับแก้ไข):**
    ...
    - หมวดหมู่ (Category - Shadcn/ui Select)
    - รูปปกนิยาย (Cover Image - อัปโหลดใหม่, Crop 3:4)
    - ภาษาต้นฉบับ (Original Language - Shadcn/ui Select: zh, en, th)
    - **Slug นิยาย (`novel_slug`)**: (Shadcn/ui Input) สามารถแก้ไขได้ (ต้องมี real-time validation check ความซ้ำซ้อนผ่าน API call หรือ on blur)
    - สถานะนิยาย (Novel Status - Shadcn/ui Select/RadioGroup: "ต่อเนื่อง", "จบแล้ว", "ยุติการลง")
    - สิทธิ์ในการมองเห็นนิยาย (Novel Visibility - Shadcn/ui Select (multiple)/Checkbox Group)
  - **ค่าเริ่มต้นสำหรับการสร้างตอนใหม่ของนิยายนี้ (Default Chapter Settings for this Novel - เหมือนตอนสร้าง):**
    - สถานะเริ่มต้นของตอน (Default Chapter Status - Shadcn/ui RadioGroup)
    - สิทธิ์การเข้าถึงตอนเริ่มต้น (Default Chapter Access - Shadcn/ui Select (multiple)/Checkbox Group)
  - **ปุ่มดำเนินการ (Shadcn/ui Button):**
    - "บันทึกการเปลี่ยนแปลง": Submit form, เรียก API Route เพื่ออัปเดตข้อมูลใน `novels` table (เมื่อสำเร็จ แสดง Toast "บันทึกข้อมูลนิยาย '[ชื่อนิยาย]' สำเร็จแล้ว", และสร้างรายการใหม่ใน `novel_versions` หากมีการเปลี่ยนแปลงข้อมูลสำคัญ, ปิด Modal, React Query invalidate query เพื่อรีเฟรชข้อมูลหน้า)
    - "ยกเลิก" (ปิด Modal)
    - (อาจมี) ปุ่ม "ลบนิยาย" ภายใน Modal นี้ด้วย (Shadcn/ui Button, destructive): แสดง AlertDialog ยืนยัน, Toast, redirect
...
- **UI ภายใน Modal (Shadcn/ui Dialog):**
  - **หัวเรื่อง Modal:** "เพิ่มหลายตอนด้วยไฟล์ .txt สำหรับนิยาย: [ชื่อนิยาย]"
  - **พื้นที่ Drag & Drop ไฟล์ หรือ ปุ่ม "เลือกไฟล์" (Shadcn/ui Input type file, multiple):**
  ...
  - **ตัวเลือกการจัดการไฟล์ชื่อซ้ำ (ถ้า `chapter_number` ที่ได้จากชื่อไฟล์ซ้ำกับที่มีอยู่):**
    - (Shadcn/ui RadioGroup): "เขียนทับตอนที่มีอยู่ (Overwrite)" หรือ "ข้ามไฟล์นี้ (Skip)"
  - **Progress Bar (Shadcn/ui Progress):** แสดงความคืบหน้าการอัปโหลดและประมวลผลไฟล์ (ถ้าทำได้แบบ real-time)
...
- **การทำงานเบื้องหลัง (Next.js API Route ที่รับ `multipart/form-data`):**
  ...
  - ประมวลผลแต่ละไฟล์:
    ...
    - **สำคัญ:** สำหรับแต่ละตอนที่สร้างใหม่สำเร็จ, สร้าง record ว่างๆ ในตาราง `mark_lines` (สำหรับนักแปลเจ้าของนิยาย) และสร้าง `chapter_versions` record แรกโดยอัตโนมัติ
- **การแจ้งเตือน (Toast) เมื่อสำเร็จ:** สรุปผลอย่างละเอียด เช่น "เพิ่มตอนใหม่ X ตอนสำเร็จ, เขียนทับ Y ตอน, ข้าม Z ตอน"
- **การพัฒนา:** Shadcn/ui Dialog, Input, Progress, Next.js API Route (ใช้ library เช่น `formidable` หรือ `multer` สำหรับ parsing `multipart/form-data` ถ้าจำเป็น), Supabase (batch insert `chapters`, `mark_lines`, `chapter_versions`)
...
- **UI ภายใน Modal (Shadcn/ui Dialog):**
  ...
  - **ฟอร์ม (React Hook Form + Zod):**
    - **ชื่อตอน (Chapter Title):** (Shadcn/ui Input), จำเป็นต้องกรอก (Zod: `required`)
    - **หมายเลขตอน (`chapter_number` - เก็บเป็น TEXT):**
      - **เพิ่มตอนใหม่:** (Shadcn/ui Input) แสดงค่า auto-increment จากตอนล่าสุด +1 เป็นค่าเริ่มต้น (เช่น ถ้าตอนล่าสุดคือ "005" ให้แสดง "006"). **ผู้ใช้สามารถแก้ไขได้** (ต้อง validate ไม่ให้ซ้ำกับ `chapter_number` อื่นในนิยายเดียวกัน และควรเป็นรูปแบบตัวเลขที่อาจมี padding).
      - **แก้ไขตอน:** (Shadcn/ui Input) แสดงเลขปัจจุบัน, **ผู้ใช้สามารถแก้ไขได้** (ต้องระวังเรื่องลำดับ และ validate ความซ้ำซ้อน).
    ...
    - **สถานะตอน (Chapter Status):** (Shadcn/ui RadioGroup) ("เผยแพร่", "ฉบับร่าง")
    - **สิทธิ์การเข้าถึงตอน (Chapter Access Level):** (Shadcn/ui Select (multiple) หรือ Checkbox Group) ("ทุกคน", "เฉพาะสมาชิก", "เฉพาะนักแปล")
    - *หมายเหตุ:*
...
- **การทำงานเบื้องหลัง (Next.js API Route):**
  - **เมื่อเพิ่มตอนใหม่สำเร็จ:** สร้าง record ใน `chapters`, สร้าง record ว่างๆ ใน `mark_lines` (สำหรับนักแปลเจ้าของนิยาย), และสร้าง `chapter_versions` record แรก. แสดง Toast "เพิ่มตอน '[ชื่อตอน]' สำเร็จแล้ว". React Query invalidate query สารบัญ.
  - **เมื่อแก้ไขตอนสำเร็จ:** อัปเดต record ใน `chapters`. หากมีการเปลี่ยนแปลงเนื้อหา (`content_original`) หรือข้อมูลสำคัญอื่นๆ (title, status, access) ให้สร้างรายการใหม่ใน `chapter_versions`. แสดง Toast "บันทึกการเปลี่ยนแปลงตอน '[ชื่อตอน]' เรียบร้อยแล้ว". React Query invalidate query สารบัญ.
- **การพัฒนา:** Shadcn/ui Dialog, Input, RadioGroup, Select, React Hook Form + Zod, TipTap, Next.js API Route, Supabase
...
- **ความสามารถ:**
  ...
  - **สำหรับแต่ละนิยายในรายการ (แสดงเป็น Shadcn/ui Card หรือ List Item):**
    ...
    - ปุ่ม "ไปยังบุ๊คมาร์คล่าสุด" (Shadcn/ui Button หรือ Link): นำทางผู้ใช้ไปยังหน้าอ่านของตอนล่าสุดที่บุ๊คมาร์คสำหรับนิยายเรื่องนั้น (ใช้ `<Link href={`/novels/${novelId}/${latestBookmarkedChapterNumber}`}>`)
    - ปุ่ม "จัดการบุ๊คมาร์คสำหรับเรื่องนี้" (Shadcn/ui Button): เปิด Modal "จัดการบุ๊คมาร์คสำหรับเรื่อง: [ชื่อนิยาย]"
- **Modal "จัดการบุ๊คมาร์คสำหรับเรื่อง: [ชื่อนิยาย]" (Shadcn/ui Dialog):**
  - **หัวเรื่อง:** "จัดการบุ๊คมาร์คสำหรับ: [ชื่อนิยาย]"
  - **ตัวเลือกการเรียงลำดับภายใน Modal:** (Shadcn/ui Select) (เรียงตามลำดับตอน (น้อยไปมาก/มากไปน้อย), เรียงตามวันที่บุ๊คมาร์ค (ล่าสุด/เก่าสุด))
  - **รายการบุ๊คมาร์คของเรื่องนั้น (แสดงเฉพาะตอนที่บุ๊คมาร์คไว้, เป็น List พร้อม Shadcn/ui Checkbox):**
    - Checkbox หน้าแต่ละรายการ (สำหรับเลือกเพื่อลบ)
    - หมายเลขตอน, ชื่อตอน
    - วันที่/เวลาที่บุ๊คมาร์ค
  - **ปุ่มดำเนินการใน Modal (Shadcn/ui Button):**
    - ปุ่ม "ลบรายการที่เลือก": เรียก API Route เพื่อลบบุ๊คมาร์คที่ผู้ใช้ติ๊กเลือก (เมื่อสำเร็จ แสดง Toast "ลบบุ๊คมาร์ค X รายการสำเร็จแล้ว" และ React Query invalidate query บุ๊คมาร์ค)
    - ปุ่ม "ล้างบุ๊คมาร์คทั้งหมดของเรื่องนี้":
      - แสดง AlertDialog ยืนยัน
      - เมื่อยืนยัน, เรียก API Route เพื่อลบบุ๊คมาร์คทั้งหมดของนิยายเรื่องนั้นสำหรับผู้ใช้ปัจจุบัน (เมื่อสำเร็จ แสดง Toast "ลบบุ๊คมาร์คทั้งหมดของเรื่อง '[ชื่อนิยาย]' สำเร็จแล้ว", ปิด Modal, React Query invalidate)
    - ปุ่ม "ปิด" หรือ "ยกเลิก"
- **ปุ่ม "ลบบุ๊คมาร์คทั้งหมด (ทุกเรื่อง)":**
  - (Shadcn/ui Button, destructive variant) วางในตำแหน่งที่เหมาะสม
  - เมื่อคลิก แสดง AlertDialog ยืนยัน "คุณต้องการลบบุ๊คมาร์คทั้งหมดทุกเรื่องใช่หรือไม่?"
  - เมื่อยืนยัน, เรียก API Route เพื่อลบบุ๊คมาร์คทั้งหมดของผู้ใช้ปัจจุบัน (เมื่อสำเร็จ แสดง Toast "ลบบุ๊คมาร์คทั้งหมด (ทุกเรื่อง) เรียบร้อยแล้ว" และ React Query invalidate)
- **การพัฒนา:** Next.js Page, React Query, Supabase (API calls ผ่าน Next.js API Routes), Shadcn/ui (Card, List, Dialog, AlertDialog, Select, Checkbox, Button), Lucide React (Icons)
...
- **ความสามารถ:**
  ...
  - **รูปแบบการแสดงผล: การ์ดนิยาย (Shadcn/ui Card):**
    - รูปปก (`next/image`), ชื่อนิยาย
    - **(เพิ่มเติม)** ความคืบหน้าการมาร์คโดยรวมสำหรับนิยายนั้นๆ (เช่น "X% ของตอนทั้งหมดมีมาร์ค" หรือ "Y/Z ตอนมีมาร์ค") พร้อม (Shadcn/ui Progress Bar) ที่แสดงเปอร์เซ็นต์นี้ (Logic การคำนวณอาจทำใน API Route หรือ Client-side จากข้อมูลตอนทั้งหมดและตอนที่มีมาร์ค)
  - **สำหรับแต่ละนิยายในการ์ด:**
    - ปุ่ม "จัดการบรรทัดมาร์ค" (Shadcn/ui Button): นำทางไปยังหน้า `/translator/manage-marks/[novel_id]` (ใช้ `<Link>`)
    - ปุ่ม "ไปยังหน้าข้อมูลนิยาย" (Shadcn/ui Button): นำทางไป `/novels/[novel_id]`
    - (อาจมี) ปุ่ม "เพิ่มตอนใหม่" (เปิด Modal 5.2.3)
- **การพัฒนา:** Next.js Page, React Query, Supabase (API calls ผ่าน Next.js API Routes), Shadcn/ui (Card, Progress Bar, Button)
...
- **ส่วนควบคุมหลัก:**
  - ปุ่ม "เลือกตอนเพื่อดำเนินการหลายรายการ" (Shadcn/ui Button หรือ Switch) (Toggle Batch Mode): เมื่อเปิดโหมดนี้ จะแสดง Checkbox หน้าแต่ละตอน และแสดง Batch Actions Toolbar
- **การแสดงผลสารบัญตอน (Shadcn/ui Accordion หรือ List):**
  - ปรับจำนวนตอนต่อกลุ่ม (10, 20, 50, 100 ตอน)
  - หัวกลุ่มตอน: ชื่อกลุ่ม (เช่น "ตอนที่ 1-50"), (ถ้า Batch Mode) Checkbox "เลือกทั้งหมดในกลุ่มนี้"
- **รายการตอน (สำหรับแต่ละตอนในสารบัญ):**
  - (ถ้า Batch Mode) (Shadcn/ui Checkbox) สำหรับเลือกตอน
  - หมายเลขตอน, ชื่อตอน
  - จำนวนบรรทัดที่มาร์คในตอนนี้ (นับจาก `mark_lines.marked_line_numbers` JSONB array length)
  - สถานะการมาร์ค (เช่น ไอคอน `CheckCircle2`/`Circle` จาก Lucide React)
  - ปุ่ม "คัดลอกมาร์คตอนนี้" (ไอคอน `Copy` จาก Lucide React, Shadcn/ui Button): คัดลอกรายการเลขบรรทัดที่มาร์คของตอนนี้ไปยัง Clipboard (ใช้ `navigator.clipboard.writeText`), แสดง Toast
  - ปุ่ม "ไปที่หน้าอ่าน/แปลตอนนี้": นำทางไปหน้าอ่านของตอนนี้ในโหมดนักแปล (ใช้ `<Link>`)
- **Batch Actions Toolbar (Shadcn/ui Toolbar, แสดงเมื่อ Batch Mode และมีตอนที่เลือก):**
  - ข้อความแสดงจำนวนตอนที่เลือก
  - ปุ่ม "คัดลอกบรรทัดมาร์คของตอนที่เลือก": คัดลอกมาร์คของทุกตอนที่เลือก, แสดง Toast
  - ปุ่ม "ดาวน์โหลดไฟล์บรรทัดมาร์คของตอนที่เลือก":
    - เปิด Modal (Shadcn/ui Dialog) ตัวเลือกการดาวน์โหลด:
      - **รูปแบบไฟล์:** (Shadcn/ui RadioGroup) "รวมเป็นไฟล์ .txt เดียว" หรือ "แยกไฟล์ .zip"
    ...
    - เมื่อผู้ใช้เลือกและกดยืนยัน: Client-side logic (ใช้ JSZip สำหรับ .zip) สร้างและเริ่มดาวน์โหลดไฟล์, แสดง Toast
- **การพัฒนา:** Next.js Page (Dynamic Route), React Query, Shadcn/ui (Accordion, Checkbox, Button, Modal, Toolbar, RadioGroup), Lucide React, Supabase (API calls), JSZip (สำหรับสร้าง .zip client-side)
...
- **การแสดงผลร่วม (ทั้ง PC และ Mobile):**
  - **ปุ่ม "Arrow Up" (กลับไปด้านบนสุด):** (ไอคอน `ArrowUp` จาก Lucide React) แสดงเมื่อเลื่อนลง, กดแล้ว smooth scroll to top
...
- **โหมดการอ่าน (สำหรับ Member และ Translator เมื่อไม่ได้เปิด "โหมดนักแปล"):**
  - **แถบเครื่องมือหลักด้านบน (Top Reading Toolbar - React Component, อาจเป็น sticky):**
    - **PC Items:**
      - ปุ่ม "สารบัญ" (ไอคอน `List`): เปิด Modal (Shadcn/ui Dialog) แสดงรายการตอน
      - ปุ่ม "ปรับแต่งการแสดงผล (Aa)" (ไอคอน `Settings2`): เปิด Popover/Dropdown (Shadcn/ui) ตั้งค่า Font Size, Line Height (สถานะเก็บใน Zustand/Context + localStorage)
      - ปุ่ม "บุ๊คมาร์คตอนนี้" (ไอคอน `Bookmark`): สลับสถานะ (API call), แสดง Toast
      - ปุ่ม "สลับธีมเว็บไซต์" (ไอคอน `Sun`/`Moon`)
      - ปุ่ม "โหมดอ่านต่อเนื่อง" (ไอคอน `Infinity`): เปิด/ปิด (สถานะเก็บใน Zustand/Context + localStorage), Toast
...
- **โหมดนักแปล (Translator Mode - แสดงผลเมื่อนักแปลเปิด "โหมดนักแปล" สำหรับนิยายนี้):**
  - **แถบเครื่องมือหลักด้านบน (Top Reading Toolbar):** เหมือนโหมดอ่าน แต่ **เพิ่มเติมด้วย**:
    - ปุ่ม "คัดลอกมาร์คทั้งหมด" (ไอคอน `CopyCheck`): สำหรับตอนปัจจุบัน, แสดง Toast
  - **การแสดงผลเนื้อหา (แตกต่างจากโหมดอ่าน):**
    ...
    - **Checkbox สำหรับมาร์ค (Shadcn/ui Checkbox):** แสดงทางขวาสุดของแต่ละบรรทัด
  - **MarkLine Interaction:**
    - **คลิก Checkbox:** มาร์ค/ยกเลิกมาร์คบรรทัดนั้นๆ. **Real-time update:** เรียก API Route ทันทีเพื่ออัปเดต `mark_lines.marked_line_numbers` ใน Supabase (อาจใช้ `useMutation` จาก React Query). แสดง Toast "มาร์ค/ยกเลิกมาร์คบรรทัดที่ X".
    - **ดับเบิลคลิกที่บรรทัดเนื้อหา:**
      ...
      - แสดง Toast "คัดลอกและมาร์คบรรทัดที่ X แล้ว"
  - **เมื่อเลื่อนถึงท้ายบทในโหมดนักแปล:**
    ...
    - ปุ่ม "ล้างมาร์คทั้งหมดของตอนนี้" (Shadcn/ui Button, destructive): แสดง AlertDialog ยืนยัน, เรียก API update, Toast
    - ปุ่ม "คัดลอกมาร์คทั้งหมดของตอนนี้" (Shadcn/ui Button)
- **การพัฒนา:**
  ...
  - Shadcn/ui (Dialog, Popover, Button, Checkbox, Switch, AlertDialog), Lucide React
  - Supabase (API calls ผ่าน Next.js API Routes หรือ Server Actions ถ้า Next.js เวอร์ชันใหม่รองรับดี), React Query (`useQuery`, `useMutation`)
...
- **โครงสร้างหน้าแดชบอร์ด (Admin Layout Component - `components/layouts/AdminLayout.tsx`):**
  - **Sidebar นำทางด้านซ้าย (Admin Sidebar - สร้างด้วย Shadcn/ui NavigationMenu หรือ List):**
    - โลโก้/ชื่อเว็บไซต์
...
- **ความสามารถ:** แสดงข้อมูลสรุปในรูปแบบ (Shadcn/ui Card) หรือกราฟเบื้องต้น (ถ้าต้องการ อาจใช้ `recharts` หรือ `nivo`):
  - จำนวนนิยายทั้งหมด (แบ่งตามสถานะ)
  - จำนวนตอนทั้งหมด
  - จำนวนผู้ใช้ทั้งหมด (แบ่งตามบทบาท)
  - จำนวนผู้ใช้ลงทะเบียนใหม่ (เช่น ใน 7 วัน/30 วัน)
- **การพัฒนา:** Next.js Page, Shadcn/ui Card, Supabase (API calls)
...
- **หน้า `/admin/dashboard/users/index.tsx` (List Users):**
  ...
  - **ส่วนควบคุม:**
    - ช่องค้นหาผู้ใช้ (Shadcn/ui Input): ค้นหาตาม Email, ชื่อที่แสดง (กรอง real-time debounced หรือ on-submit)
    - ฟิลเตอร์ตามบทบาท (Shadcn/ui Select)
    - แสดงจำนวนผู้ใช้ทั้งหมด/ตามผลค้นหา
  - **ตารางผู้ใช้ (User Table - สร้างด้วย Shadcn/ui Table):**
    - **คอลัมน์:** `#`, Avatar (`next/image`), Email, ชื่อที่แสดง, สิทธิ์ (Shadcn/ui Badge สีตามบทบาท), วันที่สมัคร, วันหมดอายุสิทธิ์, สถานะ (ปกติ/ถูกระงับ), ปุ่ม "จัดการ" (ไอคอน `Settings` หรือ `Edit` จาก Lucide React, ลิงก์ไป `/admin/dashboard/users/[user_id]`)
    - หัวคอลัมน์ Sortable, มี Pagination (Shadcn/ui Pagination)
- **หน้า `/admin/dashboard/users/[user_id].tsx` (Edit User Detail Page):**
  ...
  - **UI (Form จัดการด้วย React Hook Form + Zod):**
    ...
    - Email (Read-only)
    - ชื่อที่แสดง (Display Name - Shadcn/ui Input สำหรับแก้ไข)
    - สิทธิ์ (Role - Shadcn/ui Select สำหรับแก้ไข)
    - วันที่สมัคร (Read-only)
    - วันที่สิทธิ์หมดอายุ (Role Expiry Date - Shadcn/ui DatePicker + Popover, ปุ่มช่วย: +1/3/6 เดือน, ไม่มีหมดอายุ)
    - ระงับบัญชี (Suspend Account - Shadcn/ui Switch)
  - **ปุ่มดำเนินการ (Shadcn/ui Button):**
    - "บันทึกการเปลี่ยนแปลง": เรียก API Route เพื่ออัปเดต `profiles` (Toast, React Query invalidate list)
    - "ยกเลิก" (กลับไปหน้ารายการผู้ใช้)
- **การพัฒนา:** Next.js Pages (Dynamic Routes), Shadcn/ui (Input, Table, Pagination, Select, DatePicker, Switch, Badge), React Hook Form + Zod, Supabase (API calls), React Query
...
- **ส่วนเพิ่ม/แก้ไขหมวดหมู่ (ใช้ Shadcn/ui Dialog + Form React Hook Form + Zod):**
  - ปุ่ม "เพิ่มหมวดหมู่ใหม่" (เปิด Dialog)
  - **Dialog เพิ่ม/แก้ไขหมวดหมู่:**
    - ฟอร์ม: ชื่อหมวดหมู่ (จำเป็น, unique), Slug (auto-generate, แก้ไขได้, unique), คำอธิบาย (optional)
    - ปุ่ม: "บันทึก", "ยกเลิก" (Toast, React Query invalidate)
- **รายการหมวดหมู่ (Shadcn/ui Table หรือ List):**
  - แสดง: ชื่อ, Slug, จำนวนนิยายในหมวด
  - ปุ่ม "แก้ไข" (เปิด Dialog แก้ไข)
  - ปุ่ม "ลบ" (Shadcn/ui Button, destructive): แสดง AlertDialog ยืนยัน, **ตัวเลือกจัดการนิยายในหมวดที่จะลบ** (ย้ายไปหมวดอื่น (Select) หรือตั้งเป็น NULL), เรียก API Route (Toast, React Query invalidate)
- **การพัฒนา:** Next.js Page, Shadcn/ui (Dialog, Table, Button, Select, AlertDialog), React Hook Form + Zod, Supabase (API calls), React Query
...
- **ความสามารถ:**
  - ปุ่ม "สร้างประกาศใหม่" (เปิด Dialog สร้าง/แก้ไขประกาศ)
  - **ตารางประกาศ (Shadcn/ui Table):**
    - คอลัมน์: หัวข้อ, สถานะ (Active/Inactive - Shadcn/ui Badge), หน้าที่แสดง, กลุ่มผู้รับ, วันที่เริ่ม/สิ้นสุด, ปุ่มดำเนินการ
    - ปุ่มดำเนินการ: "แก้ไข" (เปิด Dialog), "ลบ" (AlertDialog, Toast), "สลับสถานะ" (Switch หรือ Button, API call, Toast)
- **Dialog สร้าง/แก้ไขประกาศ (Shadcn/ui Dialog + Form React Hook Form + Zod):**
  - ฟิลด์: หัวข้อ, เนื้อหา (Markdown - TipTap หรือ `react-markdown` editor/preview), วันที่เริ่ม/สิ้นสุด (DatePicker), หน้าที่แสดง (Multi-select/Checkbox Group), กลุ่มผู้รับ (Multi-select/Checkbox Group), สถานะ "ใช้งาน" (Switch)
  - ปุ่ม: "บันทึก", "ยกเลิก" (Toast, React Query invalidate)
- **การพัฒนา:** Next.js Page, Shadcn/ui (Dialog, Table, Button, Select, DatePicker, Switch, Badge), React Hook Form + Zod, TipTap/`react-markdown`, Supabase (API calls), React Query
...
- **Layout:** แบ่งเป็น Section หรือ Card (Shadcn/ui Card)
- **UI (Form จัดการด้วย React Hook Form + Zod, อาจมีหลาย Form หรือ Form เดียวใหญ่):**
  ...
  - **ส่วนลงทะเบียน/เข้าสู่ระบบ:** เปิด/ปิดการลงทะเบียน (Switch), เปิด/ปิด Google OAuth (Switch)
  - **ส่วนเนื้อหา:** จำนวนรายการต่อหน้าเริ่มต้น, ค่าเริ่มต้นสำหรับนิยายใหม่ (สถานะตอน, สิทธิ์เข้าถึงตอน)
- **ปุ่มดำเนินการ (Shadcn/ui Button):** "บันทึกการตั้งค่า" (เรียก API Route เพื่ออัปเดต `site_settings`, Toast)
- **การพัฒนา:** Next.js Page, Shadcn/ui (Card, Input, Textarea, Switch, File Input, Button), React Hook Form + Zod, Supabase (API calls), React Query
...
- **Layout:** ใช้ (Shadcn/ui Tabs) แยกประเภทเวอร์ชัน
- **Tab 1: เวอร์ชันเว็บไซต์ / Changelog:**
  - **Data Fetching:** React Query สำหรับ `site_changelogs`
  - แสดงรายการ Changelog (Shadcn/ui Table): เลขเวอร์ชัน, วันที่, สรุป (render Markdown), สถานะ
  - ปุ่ม "เพิ่ม Changelog ใหม่" (เปิด Dialog)
  - **Dialog เพิ่ม/แก้ไข Changelog (Form React Hook Form + Zod):** เลขเวอร์ชัน, วันที่, สรุป (Markdown), สถานะ "เผยแพร่" (Switch)
  - ปุ่มดำเนินการต่อรายการ: "แก้ไข", "ลบ" (AlertDialog)
- **Tab 2: เวอร์ชันนิยายและตอน:**
  - **Data Fetching:** React Query (สำหรับค้นหานิยาย, ดึง `novel_versions`, `chapter_versions`)
  - ส่วนค้นหานิยาย (Shadcn/ui Input หรือ Select)
  - **เมื่อเลือกนิยาย:**
    - **ประวัติเวอร์ชันข้อมูลเมตานิยาย (Shadcn/ui Table):** แสดงรายการจาก `novel_versions`: เวอร์ชัน, วันที่, ผู้แก้ไข, หมายเหตุ. ปุ่ม "ดูรายละเอียด" (Dialog แสดง Snapshot JSONB), ปุ่ม "กู้คืนเวอร์ชันนี้" (AlertDialog ยืนยัน, เรียก API, Toast)
    - **ประวัติเวอร์ชันตอน:**
      - (Shadcn/ui Select) เลือกตอนของนิยายนั้น
      - **เมื่อเลือกตอน (Shadcn/ui Table):** แสดงรายการจาก `chapter_versions`: เวอร์ชัน, วันที่, ผู้แก้ไข, หมายเหตุ. ปุ่ม "ดูเนื้อหา" (Dialog แสดง Snapshot), ปุ่ม "กู้คืนเวอร์ชันนี้" (AlertDialog, API, Toast)
- **การพัฒนา:** Next.js Page, Shadcn/ui (Tabs, Table, Dialog, Input, DatePicker, Switch, Select, AlertDialog), React Hook Form + Zod, `react-markdown` (สำหรับแสดง Markdown), Supabase (API calls), React Query

### 5.7 Modal สร้างนิยาย (Create Novel Modal)
- (รายละเอียดตามข้อ 5.1.1 ทุกประการ, ใช้ Shadcn/ui Dialog, React Hook Form + Zod, TipTap, react-image-crop)

### 5.8 การจัดการตอน (Chapter Management)
- **5.8.1 Modal เพิ่มตอนด้วย .txt (Batch Upload Modal):** (รายละเอียดตามข้อ 5.2.2, ใช้ Shadcn/ui Dialog)
- **5.8.2 Modal เพิ่ม/แก้ไขตอน (Create/Edit Chapter Modal):** (รายละเอียดตามข้อ 5.2.3, ใช้ Shadcn/ui Dialog, React Hook Form + Zod, TipTap)
...
- **Layout:** หน้าเพจเดียว, แบ่งเป็นส่วนๆ (Shadcn/ui Card)
- **ความสามารถ (ใช้ Shadcn/ui Components และ React Hook Form + Zod สำหรับฟอร์ม):**
  - **รูปโปรไฟล์ (Avatar):**
    - แสดง Avatar ปัจจุบัน (`next/image` จาก `profiles.avatar_url`)
    - ปุ่ม "เปลี่ยนรูปโปรไฟล์": เปิด File Picker, ใช้ `react-image-crop` Crop 1:1, อัปโหลดผ่าน API Route (Toast)
  - **ชื่อที่แสดง (Display Name):**
    - แสดงชื่อปัจจุบัน
    - (Shadcn/ui Input) สำหรับแก้ไข + ปุ่ม "บันทึกชื่อที่แสดง" (API call, Toast)
  - **สิทธิ์ผู้ใช้งาน (Role):**
    - แสดงบทบาทปัจจุบัน (Shadcn/ui Badge สีตามบทบาท), Read-only
  - **อีเมล (Email):**
    - แสดงอีเมล (Read-only)
  - **เปลี่ยนรหัสผ่าน (Change Password):**
    - ปุ่ม "เปลี่ยนรหัสผ่าน" -> เปิด Modal (Shadcn/ui Dialog)
    - **ภายใน Modal (Form React Hook Form + Zod):** รหัสผ่านปัจจุบัน, รหัสผ่านใหม่, ยืนยันรหัสผ่านใหม่
    - ปุ่ม: "บันทึกรหัสผ่านใหม่", "ยกเลิก" (เรียก Supabase Auth API, Toast "เปลี่ยนรหัสผ่านสำเร็จแล้ว", **บังคับ Logout และ Redirect ไปหน้า Login**)
  - **ออกจากระบบ (Logout):**
    - ปุ่ม "ออกจากระบบ" (Shadcn/ui Button) -> เปิด AlertDialog ยืนยัน (เรียก Supabase `signOut()`, Toast "ออกจากระบบแล้ว", Redirect ไปหน้าแรก)
- **การพัฒนา:** Next.js Page, Shadcn/ui (Card, Avatar, Input, Button, Dialog, AlertDialog, Badge), React Hook Form + Zod, Supabase (Auth API, DB API), `react-image-crop`



## 6. Key User Experience Flows & Enhancements

- **การล็อกอิน/ลงทะเบียน:** ราบรื่นผ่าน Modal (Shadcn/ui Dialog), มี Feedback ชัดเจน (Toast)
- **การนำทาง (Navigation):** โครงสร้าง Navbar (Shadcn/ui NavigationMenu หรือ custom) และ Admin Sidebar ชัดเจน, URL เป็นมิตร (Next.js Routing)
...
- **เครื่องมือช่วยแปล (Translator Tools):**
  ...
  - การมาร์คบรรทัด (คลิก checkbox, ดับเบิลคลิกบรรทัด) **ต้อง Real-time และตอบสนองทันที** (ใช้ `useMutation` จาก React Query ส่งการเปลี่ยนแปลงไปยัง API Route เพื่ออัปเดต `mark_lines` โดยอัตโนมัติพร้อม Toast feedback)
...
- **การตอบสนองของระบบ (System Responsiveness):**
  ...
  - **Loading States:** แสดง Spinner (Lucide React) หรือ Skeleton UI (Shadcn/ui Skeleton หรือ `react-loading-skeleton`) ขณะโหลดข้อมูล (จัดการผ่าน State ของ React Query หรือ SWR)
  - **Error Handling:** แสดงข้อความข้อผิดพลาดที่เป็นมิตร (Toast หรือใน UI) เมื่อเกิดปัญหา (จัดการผ่าน State ของ React Query หรือ `catch` block)
- **การออกแบบที่สวยงามและตอบสนองทุกอุปกรณ์:** ตามปรัชญาการออกแบบ (ใช้ Tailwind CSS และ Shadcn/ui)
- **ปุ่ม Arrow Up:** สำหรับเลื่อนกลับไปบนสุดในหน้าที่มีการ scroll ยาวๆ (หน้าอ่าน, รายการยาวๆ)
- **การแจ้งเตือนการกระทำ (Action Confirmations):** ใช้ Toast notifications (Shadcn/ui Toaster/Sonner) แสดงผลข้อความยืนยันการกระทำต่างๆ
- **การสร้าง `mark_lines` อัตโนมัติ:** เมื่อมีการสร้างตอนใหม่ (ทั้งแบบ Manual หรือ Batch Upload) ระบบ (API Route ที่สร้างตอน) จะต้องสร้าง record ว่างๆ (`marked_line_numbers: []`) ในตาราง `mark_lines` สำหรับตอนนั้นและนักแปลเจ้าของนิยาย (`created_by` ของนิยาย) โดยอัตโนมัติ
- **การคงสถานะหน้าเมื่อรีเฟรช (Persistent State on Refresh):** ทุกหน้าที่มี URL เฉพาะ (เช่น หน้าอ่านตอน, หน้าข้อมูลนิยาย, หน้าใน Admin Dashboard) เมื่อผู้ใช้รีเฟรชเบราว์เซอร์ (F5) จะต้องยังคงอยู่ที่หน้าเดิมและแสดงข้อมูลเดิม (Next.js Routing และ Data Fetching methods เช่น `getServerSideProps` หรือ React Query với caching จัดการส่วนนี้) สถานะ UI ที่ไม่ได้อยู่ใน URL (เช่น สถานะการเปิด/ปิดของ Accordion, ค่าในฟิลเตอร์ที่ยังไม่ submit) อาจใช้ `localStorage` หรือ Global State (Zustand/Context) ที่ persist ข้อมูลบางส่วน

## 7. Database Structure (Supabase - PostgreSQL)

(โครงสร้างฐานข้อมูลยังคงเหมือนเดิมกับที่ระบุไว้ในเวอร์ชัน SvelteKit และการปรับปรุงครั้งล่าสุด เนื่องจาก Backend คือ Supabase และโครงสร้างข้อมูลไม่ขึ้นกับ Frontend Framework โดยตรง)
...
- **`profiles`** (
  ...
  -- (อาจมี) user_settings JSONB (สำหรับเก็บ preferences เช่น theme, reading settings)
...
- **`mark_lines`** (
  ...
  novel_id UUID NOT NULL REFERENCES novels(id) ON DELETE CASCADE,
  novel_title TEXT,
  chapter_number TEXT, -- Matches chapters.chapter_number type (TEXT)
  chapter_title TEXT,
  -- Core data:
...
- **UI Component Library:** **Shadcn/ui** (หรือ MUI / AntD ถ้าต้องการ Component สำเร็จรูปมากกว่า และมีความคุ้นเคย)
...
- **Backend & Database Platform:** **Supabase**
  ...
  - **Serverless Functions:** Supabase Edge Functions (สำหรับ Logic ฝั่ง Server ที่ซับซ้อน, การประมวลผลไฟล์, หรือ tasks ที่ต้องการ `service_role` key เช่น การจัดการ RLS หรือ Triggers ซับซ้อน, หรืออาจใช้ Next.js API Routes สำหรับ Logic ส่วนใหญ่)
...
- **การแสดงข้อความข้อผิดพลาดแก่ผู้ใช้ (User-Facing Error Messages):**
  - เมื่อเกิดข้อผิดพลาดจาก Server, แสดง Toast notification (Shadcn/ui Toaster/Sonner) หรือข้อความใน UI ที่เป็นมิตร
...
- **การกำหนดสิทธิ์ (Authorization) - Row-Level Security (RLS):**
  ...
  - กำหนด RLS policies ใน **ทุกตาราง** ที่มีข้อมูลสำคัญ
